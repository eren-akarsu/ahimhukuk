...
export const CustomDateInput: React.FC<CustomDateInputProps> = ({ value, onChange, label, className = '' }) => {
  ...
  const CustomInput = forwardRef<HTMLInputElement, any>(({ value, onClick, onChange, placeholder }, ref) => (
    <div className="relative flex items-center cursor-pointer" onClick={onClick}>
      <input
        ...
        readOnly // Prevent mobile keyboard from opening
        className="w-full pl-3.5 pr-10 py-2.5 rounded-xl bg-white/50 dark:bg-darker-bg/80 border border-gray-300 dark:border-gold/30 text-gray-800 dark:text-white text-sm focus:outline-none focus:border-gold transition-colors cursor-pointer"
      />
